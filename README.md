...
API를 요청하는 경험도 필요했는데, 이번 프로젝트를 통해 그런 경험을 많이 한 것 같아 뿌듯하는 생각도 듭니다. Riot api key는 개발용으로 발급
받을 수 있었는데, 이 api key로 요청을 할 수 잇는 횟수가 제한이 있어 많은 양의 데이터를 다루지 못해 좀 더 세세하고 다양한 기능을 구현하지 못했던
점이 아쉬웠던 것 같습니다.
API 명세서를 읽고 개발해보는 것이 이번이 처음이라 <U>잘못된 요청 정보를 전송</U>하는 경우도 많았고, 
